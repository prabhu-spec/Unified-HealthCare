...
export default function ForgotEmailPage() {
  ...
  const { forgotEmail, isAuthenticated } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    setLoading(true);
    const result = await forgotEmail(email.trim());
    setLoading(false);
    if (result.success) {
      setSent(true);
    } else {
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8 border border-gray-100 text-center">
          ...
          <p className="text-gray-600 mb-6">
            If an account exists for that address, we sent your login email there.
          </p>
          <Link
            to="/login"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8 border border-gray-100">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Forgot your email?</h1>
        <p className="text-gray-600 mb-6">
          Enter the email address you use to sign in. We’ll send you a reminder of your login email.
        </p>
        {error && (
