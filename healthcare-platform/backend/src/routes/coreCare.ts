...
import { assembleAiContextPacket, getPatientAiContext } from "../services/patientAiContext.js";
import { vitalsLatest } from "../telemetry/store.js";
import {
  bindInMemoryPatients,
  inMemoryAllergies,
  inMemoryProblems,
...
Object.assign(patients[0], {
  weightKg: 82,
  heightCm: 178,
  bloodType: "A+",
  chiefComplaint: "Persistent cough and mild chest discomfort for 5 days",
...
const inMemoryPrescriptions = [
  { id: "rx-1", patientId: "patient-1", medication: "Lisinopril 10mg", dosage: "Once daily", status: "Active", prescribedBy: "Dr. Sarah Johnson", createdAt: "2026-04-01T00:00:00.000Z" },
  { id: "rx-2", patientId: "patient-1", medication: "Amoxicillin 500mg", dosage: "1 tablet twice daily", status: "Active", prescribedBy: "Dr. Sarah Johnson", createdAt: "2026-05-20T00:00:00.000Z" },
  { id: "rx-3", patientId: "patient-2", medication: "Metformin 500mg", dosage: "Twice daily with meals", status: "Active", prescribedBy: "Dr. Sarah Johnson", createdAt: "2026-03-10T00:00:00.000Z" },
];
...
router.get("/api/core/patients/:id/ai-context", async (req: Request, res: Response) => {
  const patientId = req.params.id;
  if (useDatabase()) {
    const ctx = await getPatientAiContext(req, patientId);
    if (!ctx) return res.status(404).json({ error: "Patient not found." });
    return res.json({ data: ctx });
  }

  const patient = scopeRows(req, patients).find((p) => p.id === patientId) as Record<string, unknown> | undefined;
  if (!patient) return res.status(404).json({ error: "Patient not found." });
  ...
  const latestVitals = vitalsLatest.get(patientId) ?? null;

  const packet = assembleAiContextPacket({
    patientId,
    patient,
    clinical,
    allergies: inMemoryAllergies.filter((a) => a.patientId === patientId),
    problems: inMemoryProblems.filter((p) => p.patientId === patientId),
    medications: inMemoryPrescriptions.filter((m) => m.patientId === patientId && m.status === "Active"),
    ...
    latestVitals,
    vitalsTrend: latestVitals ? [latestVitals] : [],
  });
